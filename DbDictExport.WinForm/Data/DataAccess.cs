            ...
            using (SqlConnection conn = new SqlConnection(connBuilder.ConnectionString))
            {
                ...
                while (dr.Read())
                {
                    DbColumn column = new DbColumn()
                    {
                        ...
                        ForeignKey = dr["ForeignKey"].ToString() == "0" ? false : true,
                        IsIdentity = dr["IsIdentity"].ToString() == "0" ? false : true,
                        IsNullable = dr["IsNullable"].ToString() == "0" ? false : true,
                        Length = Int32.Parse(dr["CharactorLength"].ToString()),
                        PrimaryKey = dr["PrimaryKey"].ToString() == "0" ? false : true
                    };
                    list.Add(column);
                }
                conn.Close();
