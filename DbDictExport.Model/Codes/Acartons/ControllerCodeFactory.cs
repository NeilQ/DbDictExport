...
namespace DbDictExport.Core.Codes.Acartons
{
    public class ControllerCodeFactory : AbstractCodeFactory
    {
        ...
        public override StringBuilder GenerateCodes()
        {
            ...
            var indent = 0;
            // using
            codes.AppendLine("using Microsoft.AspNetCore.Authorization;");
            codes.AppendLine("using Microsoft.AspNetCore.Mvc;");
            codes.AppendLine("using Acartons.Api.Controllers.Base;");
            codes.AppendLine($"using {ModuleName};");

            // namespace
            codes.Append(Environment.NewLine);
            codes.AppendLine($"namespace Acartons.Api.Controllers");
            codes.AppendLine("{");
            ...
            indent++;
            codes.AppendLine(GetIndentStr(indent) + $": CrudControllerBase<{EntityName}, {EntityName}Dto, string, {EntityName}Save>");
            indent--;
            ...
            indent++;
            codes.Append(Environment.NewLine);
            codes.AppendLine(GetIndentStr(indent) + $"public {Inflector.MakePlural(EntityName)}Controller(I{EntityName}Service service, IControllerDependencies dependencies)");
            indent++;
            codes.AppendLine(GetIndentStr(indent) + " : base(service, dependencies)");
            indent--;
            codes.AppendLine(GetIndentStr(indent) + "{");
            codes.AppendLine(GetIndentStr(indent) + "}");
